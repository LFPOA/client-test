...
import { GitBranch, Shield, Bug, Settings, Package } from "lucide-react";
...
const Sidebar = () => {
    return (
        <div className="w-60 h-screen bg-white border-r">
            ...
            <nav className="px-2">
                {menuItems.map((item) => (
                    <NavLink key={item.path} to={item.path} className={({ isActive }) => `flex items-center gap-2 px-4 py-3 rounded-lg hover:bg-blue-50 text-sm font-medium ${isActive ? "bg-blue-100 text-blue-600 font-semibold" : "text-gray-700"}`}>
                        {item.icon}
                        {item.label}
                    </NavLink>
                ))}
